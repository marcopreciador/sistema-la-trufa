...
function POSApp() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          fetchTables(); // Reload all tables on any change
        }
      )
      .subscribe();
  ...
  const handleSendToKitchen = () => {
    ...
    // setShowSuccessModal(true); // SILENT FLOW
    ...
    toast.innerHTML = '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Enviado a cocina';
    document.body.appendChild(toast);
    setTimeout(() => toast.remove(), 3000);

    setActiveOrderId(null); // Return to tables
  };
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 md:p-8">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">La Trufa v2.0</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 p-8 text-center">
            ...
            <div className="space-y-3">
              {(lastCompletedOrder.type === 'delivery' || lastCompletedOrder.type === 'takeout') && (
                <button
